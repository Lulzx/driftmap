...
\State \textbf{P2G Transfer:} Interpolate $\zeta$, $n$ from particles to grid (B-splines)
\State \textbf{Poisson Solve:} Compute $\phi$ from $\nabla^2 \phi = \zeta$ (spectral)
\State \textbf{Velocity:} Compute $\mathbf{v} = \hat{\mathbf{z}} \times \nabla \phi$
\State \textbf{Source Terms:} $S_\zeta = \alpha(\phi - n) + \nu \nabla^2 \zeta$, \; $S_n = \alpha(\phi - n) - \kappa \partial_y \phi + D \nabla^2 n$
\State \textbf{Update Particles:} $\zeta_p \gets \zeta_p + \Delta t \cdot S_\zeta(\mathbf{x}_p)$, \; $n_p \gets n_p + \Delta t \cdot S_n(\mathbf{x}_p)$
\State \textbf{Advect:} Integrate $d\mathbf{x}_p/dt = \mathbf{v}$ (RK4)
\State \textbf{Jacobian:} Integrate $d\mathbf{J}_p/dt = \nabla\mathbf{v} \cdot \mathbf{J}_p$ (RK4)
